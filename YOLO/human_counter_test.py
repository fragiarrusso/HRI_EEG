...
def human_counter():
    ...
    if not os.path.exists(yolo_weights):
        raise FileNotFoundError("YOLO weights file not found: ", yolo_weights)
    if not os.path.exists(yolo_config):
        raise FileNotFoundError("YOLO config file not found: ", yolo_config)
    if not os.path.exists(yolo_names):
        raise FileNotFoundError("COCO names file not found: ", yolo_names)
    ...
    # Initialize video capture (if you want from a video recorded!)
    cap = cv2.VideoCapture("YOLO/human_count.mp4")
    if not cap.isOpened():
        raise IOError("Cannot open video file")


    # Initialize video capture for webcam (if you want from a webcam)
